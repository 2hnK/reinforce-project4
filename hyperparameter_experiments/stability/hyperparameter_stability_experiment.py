...
def run_single_trial(config_params, trial_num, total_iterations=10):
        ...
        for i in range(total_iterations):
            ...
            if monitor_stats:
                ...
                if cpu_avg is not None:
                    msg += f"CPU {cpu_avg:.1f}% avg"
                    if cpu_max is not None:
                        msg += f" / {cpu_max:.1f}% max"
                if ram_avg is not None:
                    msg += f", RAM {ram_avg:.1f}%"
                if gpu_avg is not None:
                    msg += f", GPU {gpu_avg:.1f}%"
                if vram_avg is not None:
                    msg += f", VRAM {vram_avg:.0f}MB"
                print(msg)
    ...
    trial_results['resource_summary'] = aggregate_resource_stats(monitor_samples)
...
def run_experiment(experiment_config, num_trials=5, total_iterations=10):
    ...
    trial_resource_summaries = [trial.get('resource_summary') for trial in experiment_results['trials'] if trial.get('resource_summary')]
    ...
    if resource_usage:
        experiment_results['statistics']['resource_usage'] = resource_usage
    experiment_results['resource_summary'] = resource_usage
    ...
    if 'resource_usage' in stats:
        ...
        cpu_line = f"CPU {ru.get('cpu_avg', 0):.1f}% avg"
        if 'cpu_max' in ru:
            cpu_line += f" / {ru['cpu_max']:.1f}% max"
        print(f"\nResource Usage: {cpu_line}")
        if 'gpu_avg' in ru:
            gpu_line = f"GPU {ru['gpu_avg']:.1f}% avg"
            if 'gpu_max' in ru:
                gpu_line += f" / {ru['gpu_max']:.1f}% max"
            print(f"                 {gpu_line}, VRAM {ru.get('vram_avg_mb', 0):.0f}MB avg")
    print(f"{'='*70}\n")
...
def main():
    ...
    for i, exp_config in enumerate(experiments, 1):
        ...
        try:
            ...
            # 중간 저장
            temp_file = results_dir / 'hyperparameter_experiments_progress.json'
